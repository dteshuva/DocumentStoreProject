...
public class DocumentPersistenceManager implements PersistenceManager<URI, Document> {
    ...
    private class DocumentSerializer implements JsonSerializer<Document> {
        ...
        public JsonElement serialize(Document document, Type type, JsonSerializationContext jsonSerializationContext) {
            ...
            jsonObject.addProperty("wordCounter", map);
            if(docTxt!=null){
                jsonObject.addProperty("text", gson.toJson(docTxt));
            }
            else{
                jsonObject.addProperty("binaryData", gson.toJson(DatatypeConverter.printBase64Binary(document.getDocumentBinaryData())));
            }
            return jsonObject;
        }
    }
    ...
    private class DocumentDeserializer implements JsonDeserializer<Document> {
        ...
        public Document deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
            ...
            Map<String, Integer> wordMap = gson.fromJson(jsonObject.get("wordCounter"), Map.class);
            if(jsonObject.get("text")==null){
                byte[]arr=gson.fromJson(jsonObject.get("binaryData"), byte[].class);
                Document d=new DocumentImpl(uri, arr);
                d.setWordMap(wordMap);
                return d;
            }
            String text=gson.fromJson(jsonObject.get("text"), String.class);
             Document d=new DocumentImpl(uri, text,wordMap);
            return d;
        }
    }
    ...
    public Document deserialize(URI uri) throws IOException {
        ...
        Document doc = (new DocumentDeserializer()).deserialize(JsonParser.parseReader(reader), Document.class, null);
        reader.close();
        this.delete(uri);
      //  this.deleteDir();
        return doc;
